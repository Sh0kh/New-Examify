...
export default function UserResult() {
    ...
    const getMyResult = async () => {
        setLoading(true);
        try {
            const response = await $api.get(`/study-center/my-user-exam/${ID}`);
            ...
            <main className="min-h-screen">
                <section className="mt-[50px]">
                    <div className="max-w-[1200px] mx-auto px-[10px]">
                        <h1 className="text-[32px] font-[600]">{data?.user?.name} {' '} {data?.user?.surname} </h1>
                                ...
                                <thead className="bg-gray-100 text-gray-700 text-sm rounded-[20px]">
                                    <tr>
                                        ...
                                        <th className="p-4 border-b border-gray-200">
                                            <NavLink to={`/o'quv_markaz/imtihon/foydalanuvchi/${ID}/${getSectionScore("Listening")?.id}`}>
                                                Listening
                                            </NavLink>
                                        </th>
                                        <th className="p-4 border-b border-gray-200">
                                            <NavLink to={`/o'quv_markaz/imtihon/foydalanuvchi/${ID}/${getSectionScore("Reading")?.id}`}>
                                                Reading
                                            </NavLink>
                                        </th>
                                        <th className="p-4 border-b border-gray-200">
                                            <NavLink to={`/o'quv_markaz/imtihon/foydalanuvchi/${ID}/${getSectionScore("Speaking")?.id}`}>
                                                Speaking
                                            </NavLink>
                                        </th>
                                        <th className="p-4 border-b border-gray-200">
                                            <NavLink to={`/o'quv_markaz/imtihon/foydalanuvchi/${ID}/${getSectionScore("Writing")?.id}`}>
                                                Writing
                                            </NavLink>
                                        </th>
                                    </tr>
